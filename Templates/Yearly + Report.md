...
> ```dataview
> TABLE WITHOUT ID
> 	"📚" as icon, length(rows) + " جلد کتاب" as count,
> 	"`" + sum(rows.pages) + " صفحه`" as sum 
> FROM #Book
> WHERE date >= date("{{اول سال}}") AND date <= date("{{آخر سال}}")
> GROUP BY ""
> ```
> ```dataview
> TABLE WITHOUT ID
> 	"🎬" as icon, length(rows) + " فیلم" as count,
> 	"`" + round(sum(rows.length.hours),1) + " ساعت`" as sum
> FROM #Movie 
> WHERE type = "movie" AND length != null AND date >= date("{{اول سال}}") AND date <= date("{{آخر سال}}")
> GROUP BY ""
> ```
> ```dataview
> TABLE WITHOUT ID 
> "🎙" as icon,
> length(rows) + " پادکست" as count, "`" + round(sum(rows.length.hours),1) + " ساعت`" as sum
> FROM #Podcast WHERE date >= date("{{اول سال}}") AND date <= date("{{آخر سال}}")
> GROUP BY ""
> ```
> ```dataview
> TABLE WITHOUT ID
> 	"🎓" as icon, length(rows) + " دوره‌آموزشی" as count,
> 	"`" + round(sum(rows.length.hours),1) + " ساعت`" as sum
> FROM #Course 
> WHERE date >= date("{{اول سال}}") AND date <= date("{{آخر سال}}")
> GROUP BY ""
